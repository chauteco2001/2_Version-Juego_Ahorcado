...
function gameOver(errores) {
    ...
    var mensaje = (errores < 5) ? "Muy bien, la palabra es:" : "Lo sentimos, la palabra era:";
    var mensajeWidth = ctx.measureText(mensaje).width ;
    var mensajeX = (canvas.width - mensajeWidth) / 2;
    ctx.fillText(mensaje, mensajeX, 290);

    ctx.font = "bold 80px Courier";
    lon = (canvas.width - (palabra.length * 48)) / 2;
    ctx.fillText(palabra, lon, 400);
    horca(errores);
}
